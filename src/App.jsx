...
import Extension from './extensionTemplate/extension'
import './App.css'
...
import speedBoostLogo from './images/logo-speed-boost.svg';
import styleSpyLogo from './images/logo-style-spy.svg';
import tabMasterLogo from './images/logo-tab-master-pro.svg';
...
const InitialExtension = [
  ...
  },
  {
    ...
    imageURL: styleSpyLogo,
    isActive: false
  },
  {
    id:3 ,
    title: "DOM Snapshot",
    description: "Capture and analyze the structure of any webpage.",
    imageURL: domSnapshotLogo,
    isActive: false
  },
  {
    id:4 ,
    title: "Grid Guides",
    description: "Quickly visualize and analyze the grid structure of any webpage.",
    imageURL: gridGuidesLogo,
    isActive: false
  },
  {
    id:5 ,
    title: "JSON Wizard",
    description: "Edit and validate JSON data in real-time.",
    imageURL: jsonWizardLogo,
    isActive: false
  },
  {
    id:6 ,
    title: "Link Checker",
    description: "Verify the validity of links on a webpage.",
    imageURL: linkCheckerLogo,
    isActive: false
  },
  {
    id:7 ,
    title: "Markup Notes",
    description: "Add notes and annotations to HTML elements.",
    imageURL: markupNotesLogo,
    isActive: false
  },
  {
    id:8 ,
    title: "Palette Picker",
    description: "Extract and save color palettes from webpages.",
    imageURL: palettePickerLogo,
    isActive: false
  },
  {
    id:9 ,
    title: "Speed Boost",
    description: "Optimize your browser for faster loading times.",
    imageURL: speedBoostLogo,
    isActive: false
  },
  {
    id:10 ,
    title: "Tab Master Pro",
    description: "Manage and organize browser tabs efficiently.",
    imageURL: tabMasterLogo,
    isActive: false
  },
  {
    id:11 ,
    title: "Viewport Buddy",
    description: "View and control the viewport size of your browser.",
    imageURL: viewportBuddyLogo,
    isActive: false
  }
]
export default function App() {

  function toggleExten(id) {
    SetExtension(prev =>
      prev.map(exten =>
  ...
  function removeExtension(id) {
    SetExtension(prev => prev.filter(box => box.id !== id));
  }


  const filterExtension = extension.filter(exten => {
    if (filter === 'active') return exten.isActive === true;
    if (filter === 'inactive') return exten.isActive === false;
    return true; // 'all'
  });
    ...
    <>
      <div className={`extension_container  ${ChangeColor ? 'white' : ""}`}>
          ...
          <div className="extension_logo">
            <h1 className={ChangeColor ? "turndark":""}>Extension List</h1>          
            <div className="btn_container">
                ...
                <button className={filter === 'inactive' ? 'activeBtn' : ""} onClick={()=>setFilter('inactive')}>Inactive</button>
            </div>
          </div>
          <div className="extension_item">
          {filterExtension.map(exten => (
              <Extension
                key={exten.id}
                title={exten.title}
                acting={exten.isActive}
                description={exten.description}
                imageURL={exten.imageURL}
                sentRemove={() => removeExtension(exten.id)}
                sentBoolean={() => toggleExten(exten.id)}
                Color={ChangeColor}
              />
            ))}
          </div>
